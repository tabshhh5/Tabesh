...
class Tabesh_Staff {
		...
		if ( $result ) {
			...
			$wpdb->insert(
				...
				array(
					...
					'new_status'    => $status,
					'description'   => sprintf(
						__( 'وضعیت توسط %1$s از "%2$s" به "%3$s" تغییر کرد', 'tabesh' ),
	...
	public function get_assigned_orders( $staff_id = null ) {
		...
		$orders = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM $table WHERE archived = %d ORDER BY created_at DESC",
				0
			)
		);

		// Apply firewall filtering to hide confidential orders during lockdown
		$firewall = new Tabesh_Doomsday_Firewall();
		$orders   = $firewall->filter_orders_for_display( $orders, $staff_id, 'staff' );
	...
	public function user_has_staff_access_secure( $user_id ) {
		...
		$allowed_users_json = $wpdb->get_var(
			$wpdb->prepare(
