...
class Tabesh_AI_Gemini {
	...
	private function build_system_prompt( $context ) {
		...
		$prompt .= "- راهنمایی کاربران به صفحات مختلف سایت با استفاده از لیست صفحات ایندکس شده\n\n";
		...
		$prompt .= "- **مهم**: اگر کاربر خواهان رفتن به صفحه خاصی است (مثل صفحه سفارش، تماس، قیمت)، حتما از لیست صفحات موجود زیر استفاده کنید و لینک دقیق صفحه را ذکر کنید\n";
		$prompt .= "- هنگام معرفی صفحات، همیشه URL کامل و صحیح را بدون هیچ تغییری از لیست زیر استفاده کنید\n\n";
		...
		if ( ! empty( $page_list ) ) {
			$prompt .= $page_list . "\n";
			$prompt .= "**توجه مهم**: هنگام راهنمایی کاربر به صفحات، حتماً از URL های بالا استفاده کنید و لینک را تغییر ندهید.\n\n";
		}
