...
class Tabesh_Install {
    ...
    public static function update_database_schema() {
        ...
        // Add print_subtasks column to orders table for tracking print process steps
        if (self::table_exists($table_orders)) {
            if (!self::column_exists($table_orders, 'print_subtasks')) {
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log('Tabesh: Adding print_subtasks column to orders table');
                }
                
                // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                $result = $wpdb->query(
                    "ALTER TABLE `{$table_orders}` 
                    ADD COLUMN `print_subtasks` LONGTEXT DEFAULT NULL AFTER `status`"
                );
                
                if ($result === false) {
                    error_log('Tabesh: ERROR - Failed to add print_subtasks column: ' . $wpdb->last_error);
                } else {
                    error_log('Tabesh: SUCCESS - Added print_subtasks column to orders table');
                }
            }
        }
